...
function renderLeadsTable(leads, totalCount, start, end) {
    ...
    tbody.innerHTML = leads.map(lead => {
        ...
        const productsText = leadProducts.map(p => {
            ...
            return product ? `${product.title} x${p.count}` : `Товар #${p.id}`;
        }).join(', ');
        
        return `
            <tr>
                <td>${lead.id}</td>
                <td><strong>${escapeHtml(lead.client_name)}</strong><br><small style="color:#999;">${productsText.substring(0, 30)}${productsText.length > 30 ? '...' : ''}</small></td>
                ...
                <div id="products-list">
                    ${products.map(p => `
                        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 8px;">
                            ...
                            <input type="number" id="product_${p.id}" value="${getProductCount(lead, p.id)}" min="0" max="99" style="width: 60px;" placeholder="кол-во">
                        </div>
                    `).join('')}
                </div>
